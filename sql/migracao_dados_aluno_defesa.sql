...
select matricula,  str_to_date(dataTese, '%d/%m/%Y'), tituloTese, 'D', conceitoTese, resumoTese, localTese, horarioTese, numDefesa
from j17_aluno 
where 
...
select matricula, str_to_date(dataQual1, '%d/%m/%Y'), tituloQual1, 'Q1', conceitoQual1 
from j17_aluno 
where 
...
select matricula,  str_to_date(dataQual2, '%d/%m/%Y') dataq2, tituloQual2, 'Q1', conceitoQual2, resumoQual2, localQual2, horarioQual2
from j17_aluno 
where 
(str_to_date(dataQual2, '%d/%m/%Y')  <> '00-00-0000')
and 
...
insert into j17_defesa (aluno_id, data, titulo, tipoDefesa, conceito, resumo, local, horario, numDefesa)
select matricula,  str_to_date(dataTese, '%d/%m/%Y'), tituloTese, 'T', conceitoTese, resumoTese, localTese, horarioTese, numDefesa
from j17_aluno 
where 
(str_to_date(dataTese, '%d/%m/%Y')  <> '00-00-0000')
and 
