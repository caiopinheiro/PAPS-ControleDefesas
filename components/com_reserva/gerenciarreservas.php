...
function telaCadastrarReserva () { 
	...
	<div id="toolbar-box"><div class="m"><div class="toolbar-list" id="toolbar">
        <div class="cpanel2">
		    ...
		    <div class="icon" id="toolbar-save">
                <a href="index.php?option=com_reserva&Itemid=<?php echo $Itemid;?>&task=periodo">
                <span class="icon-32-stats"></span><?php echo JText::_( 'Reservar<br>Período' ); ?></a>
            </div>
            <?php } ?>
    ...
    <!-- QTDE DE RESERVAS DO USUÁRIO -->
    <?php 
		if (ehSecretaria($user->id)) $qtde = 0;
		else $qtde = qtdeReservas($user->id); 
	?>
	...
	<form method="post" name="form" action="index.php?option=com_reserva&Itemid=<?php echo $Itemid;?>">
        ...
        <fieldset>
            ...
            <select name="tipo">
				...
				<option value="Aula" <?php if($tipo == "Aula") echo "selected";?>>Aula</option>
            </select>
        ...
        <table class="table table-striped" id="tablesorter-imasters">
            <thead>
                <tr>
                    ...
                    <th>Data</th>
                    <th>Horário</th>
                    <td style="background:#E7E3E3; border-bottom:2px solid #D9D2D2; font-weight:bold;">Opções</td>
                </tr>
            </thead>
            ...
            <tbody>
            	...
            	<tr>
					...
					?>
                    	...
                    	<a href="javascript:document.form.task.value='confirmarExclusao';document.form.idReserva.value='<?php echo $reservas->id; ?>';document.form.submit()" title="Desfazer Reserva">
				...
				<div class="line-right">
                    ...
                    <div class="column-room" style="width:<?php echo $column."%";?>">
                        ...
                        foreach ($reservas as $reservaHora) { 
                            ...
                            if ($tipoSala == 'Reunião')
                                $tpSala = 'meeting'; 
                            else if ($tipoSala == 'Exame' || $tipoSala == 'Aula')
                                $tpSala = 'classes'; 
							else 
                                $tpSala = 'defense'; 								
								?>
...
<?php // CADASTRAR PERÍODO DE RESERVA : TELA 
function telaCadastrarPeriodo () { 
	$user =& JFactory::getUser();
    $database = JFactory::getDBO();
    $Itemid = JRequest::getInt('Itemid', 0);  ?>
	...
	<div id="toolbar-box"><div class="m"><div class="toolbar-list" id="toolbar">
        <div class="cpanel2">        
            <div class="icon" id="toolbar-save">
                <a href="javascript: ValidateForm(document.formData)" class="toolbar">
                    <span class="icon-32-save"></span><?php echo JText::_( 'Salvar' ); ?></a>
            </div>
        ...
        <div class="pagetitle icon-48-impressions">
            <h2>Período de Reserva de Sala</h2>
        </div>
    </div></div>
	...
	<form method="post" name="formData" action="index.php?option=com_reserva&Itemid=<?php echo $Itemid;?>" class="form-horizontal">
        ...
        <table class="table-form">            
            <tbody>
            	<tr>
                	<td>Atividade <span class="obrigatory">*</span></td>
                    <td><input type="text" name="atividade" style="width:430px;" /></td>
                </tr>
				<tr>
                	<td>Sala <span class="obrigatory">*</span></td>
                    <td><select name="sala">
                ...
                <tr>
                	<td>Dias <span class="obrigatory">*</span></td>
                    <td><div class="checkbox">
                        <input type="checkbox" id="dia1" name="dias[]" value="Segunda-Feira" />
                        <label for="dia1">Segunda-feira</label>
                        <input type="checkbox" id="dia2" name="dias[]" value="Terça-Feira" />
                        <label for="dia2">Terça-feira</label>
                        <input type="checkbox" id="dia3" name="dias[]" value="Quarta-Feira" />
                        <label for="dia3">Quarta-feira</label>
                        <input type="checkbox" id="dia4" name="dias[]" value="Quinta-Feira" />
                        <label for="dia4">Quinta-feira</label>
                        <input type="checkbox" id="dia5" name="dias[]" value="Sexta-Feira" />
                        <label for="dia5">Sexta-feira</label>
                        <input type="checkbox" id="dia6" name="dias[]" value="Sábado" />
                        <label for="dia6">Sábado</label>
                    </div></td>
                </tr>
            	<tr>
                	<td>Horário <span class="obrigatory">*</span></td>
                    <td><select name="horaInicio" style="width:100px; margin-right:15px;">
				...
				<option value="21:00">21:00</option>
                        ...
                        <select name="horaTermino" style="width:100px;">
                ...
                <tr>
                	<td>Período</td>
                    <td><input type="text" name="dataInicio" id="dataInicio" placeholder="Data Inicial" class="imgcalendar" />
    	...
    	<input type="submit" value="Submeter" name="submeter" style="display:none" />
		<input name="idSolicitante" type="hidden" value="<?php echo $user->id; ?>" />
		<input name="task" type="hidden" value="cadPeriodo" />        
    </form>
    
<?php } ?>

<?php // CADASTRAR PERÍODO DE RESERVA : SQL
function salvarPeriodo() {
	...
	$horaInicio = JRequest::getVar('horaInicio');
	$horaTermino = JRequest::getVar('horaTermino');
	$idSolicitante = JRequest::getVar('idSolicitante');
	...
	$diaInicio = explode("-", $dataInicio);
	$diaInicio = $diaInicio[2];
	
	$diaTermino = explode("-", $dataTermino);
	$diaTermino = $diaTermino[2];
	
	$mesInicio = explode("-", $dataInicio);
	$mesInicio = $mesInicio[1];
	
	$mesTermino = explode("-", $dataTermino);
	$mesTermino = $mesTermino[1];
	
	$ano = explode("-", $dataInicio);
	$ano = $ano[0];

	$time = strtotime($dataInicio);
	$timeFinal = strtotime($dataTermino);
	
	$query = "INSERT INTO `#__reservas` (`sala`, `idSolicitante`, `atividade`, tipo, `dataInicio`, `dataTermino`, `horaInicio`, `horaTermino`) VALUES ";

	$m = $d = 0;
	$funcionou = 1;
	while($time <= $timeFinal){
		$dataAtual = explode("-", date("Y-m-d",$time));
		if ($time <= $timeFinal) {
			$diaSemana = diasemana($dataAtual[2], $dataAtual[1], $dataAtual[0]);
			for ($i=0; $i < sizeof($diasArray); $i++) {
				if (strcmp($diaSemana,$diasArray[$i]) == 0) {
					$newData = date("Y-m-d", $time);

					$queryC = "SELECT * FROM #__reservas WHERE dataInicio = '$newData' AND sala = $sala";
					$database->setQuery($queryC);
					$reservas = $database->loadObjectList();
					if ($reservas) {
						foreach($reservas as $reserva) {							
							if (($reserva->horaTermino > $horaInicio) && ($reserva->horaInicio < $horaTermino)) {
							//notificarSolicitante($reserva->id);
							$funcionou = 0;
							$diasReservados = $diasReservados.", ".$newData;
//							$sql = " DELETE FROM #__reservas WHERE id = '$reserva->id'";
							//$database->setQuery($sql);
							//echo "DELETEI $newData\n";
							//$funcionou = $database->Query();
							}
						}
					}
					$query = $query .= "('$sala', '$idSolicitante', '$atividade', 'Aula', '$newData', '$newData', '$horaInicio', '$horaTermino'),";
				}
			} 
		}
	$time += 86400;			
	...
	if($funcionou){
		...
		if($funcionou) {
			echo '<div class="alert alert-success">
			  	<b>CADASTRO DE PERÍODO DE RESERVA</b><br />
				Período de Reserva de Sala cadastrado com sucesso
			  </div>';
		} else {
			echo '<div class="alert alert-error">	
			  	<b>CADASTRO DE PERÍODO DE RESERVA</b><br />
				Erro ao tentar cadastrar o período solicitado, tente novamente!
			  </div>';
		}
	}
	else{
		echo '<div class="alert alert-error">	
			  	<b>CADASTRO DE PERÍODO DE RESERVA</b><br />
				Erro ao tentar cadastrar o período solicitado. Nos dias'.$diasReservados.' esta sala já possui reservas!
			  </div>';
	}
...
<?php // NOTIFICAR SOLICITANTE
function notificarSolicitante($id) {
    global $database;
	$database =& JFactory::getDBO();

    $sql = "SELECT *, DATE_FORMAT( r.dataInicio, '%d/%m/%Y' ) AS dataFormatada 
			FROM #__users as u, #__reservas as r, #__reservas_salas as s 
			WHERE r.id = '$id' 
			AND u.id = r.idSolicitante
			AND s.id = r.sala";
    $database->setQuery($sql);
	$dados = $database->loadObjectList();

    // subject
    $subject = "NOTIFICACAO : Perda de Reserva de Sala";

    $mime_boundary = "<<<--==-->>>";

    // message
    $message .= "Prof.".$dados[0]->name."\r\n";
    $message .= "Devido à solicitação de <b>".$dados[0]->tipo."</b> pela Secretaria do IComp, a sua Reserva para a ".$dados[0]->nome." no dia ".$dados[0]->dataFormatada." foi <b>cancelada</b> por motivo de prioridade.\r\n";
