...
class ControledefesasModelListaBancas extends JModelItem
{
	    ...
	    public function filtroBanca($status_banca, $nome_aluno, $nome_orientador, $tipo_banca, $linha_pesquisa) {
			...
			$sql_standard = "SELECT d.idDefesa, bcd.id as idBanca, a.id as idAluno, bcd.status_banca, a.nome as nome_aluno, 
							M.nome as nome_orientador, d.tipoDefesa as tipo_banca, a.area as linha_pesquisa
					FROM ((((j17_aluno as a JOIN j17_defesa as d ON d.aluno_id = a.id) JOIN j17_banca_controledefesas as bcd 
					ON d.banca_id = bcd.id) JOIN j17_banca_has_membrosbanca AS MB ON bcd.id = MB.banca_id) JOIN j17_membrosbanca 
					AS M ON MB.membrosbanca_id = M.id) 
