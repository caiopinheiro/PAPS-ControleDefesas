...
class ControledefesasModelRelatorioDefesas extends JModelItem
{
	    ...
	    public function getDefesasPorPeriodoProfessor($data_inicial, $data_final, $nome_professor) {
			$database =& JFactory::getDBO();
			$sql_standard = "SELECT d.*, date_format(d.data,'%d/%m/%Y') as data_defesa, ifnull(d.conceito, '') as conceito_defesa,
							       a.nome as nome_aluno, a.id as idAluno, a.curso, p.nomeProfessor as nome_orientador, d.examinador, 
							       (case a.curso when 1 then 'Mestrado' when 2 then 'Doutorado' when 3 then 'Especial' end) as desc_curso, 
							       d.tipoDefesa, 
							       (case d.tipoDefesa 
							     		when upper('Q1') then 'Qualificação 1' 
								...
								FROM (((((
								      j17_defesa as d 
								      	LEFT JOIN j17_aluno as a ON a.id = d.aluno_id) 
							        	LEFT JOIN j17_banca_controledefesas as bcd ON d.banca_id = bcd.id AND ifnull(bcd.status_banca, '') = '1') 
										...
										LEFT JOIN j17_professores as p ON p.id = a.orientador)
								WHERE d.idDefesa is not null ";
			...
			if($nome_professor != ''){
				$sql_nome_professor = " AND (upper(mb.nome) LIKE upper('%".$nome_professor."%') OR upper(p.nomeProfessor) LIKE upper('%".$nome_professor."%') OR upper(d.examinador) LIKE upper('%".$nome_professor."%'))";
			}
