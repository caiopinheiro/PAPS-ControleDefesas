...
// Listar registros
function listarDespesasrubricas($descricao = ""){
    $database	= JFactory::getDBO();
    $Itemid = JRequest::getInt('Itemid', 0);
    $idRubrica = JRequest::getVar('idRubrica');
    $rubricaIDSaldo = identificarProjetopdID($idRubrica);
    $sql = "SELECT * FROM #__contproj_rubricadeprojeto WHERE  projeto_id LIKE  '$idRubrica' AND descricao LIKE '%$descricao%' ORDER BY descricao";
...
function excluir(form){
   ...
   if(idSelecionado > 0){
               ...
               if(resposta){
                  form.task.value = 'excluirDespesarubrica';
                  form.idDespesarubrica.value = idSelecionado;
                  form.submit();
               }
   }
   else{
...
function editar(form){
   ...
   for(i = 0;i < form.idDespesaSelec.length;i++)
        if(form.idDespesaSelec[i].checked) idSelecionado = form.idDespesaSelec[i].value;
   if(idSelecionado > 0){
           form.task.value = 'editarDespesarubrica';
           form.idDespesarubrica.value = idSelecionado;
           form.submit();
   }
   else{
...
function visualizar(form){
   ...
   for(i = 0;i < form.idDespesaSelec.length;i++)
        if(form.idDespesaSelec[i].checked) idSelecionado = form.idDespesaSelec[i].value;
   if(idSelecionado > 0){
               form.task.value = 'verDespesarubrica';//volta para visualizar projeto
               form.idDespesarubrica.value = idSelecionado;
               form.submit();
   }
   else{
...
<form method="post" name="form" enctype="multipart/form-data" action="index.php?option=com_controleprojetos&Itemid=<?php echo $Itemid;?>" method="post" >
    ...
    <div id="toolbar-box"><div class="m"><div class="toolbar-list" id="toolbar">
          <div class="cpanel2">
				<div class="icon" id="toolbar-new">
           		<a href="javascript:document.form.task.value='addDespesarubrica';document.form.submit()">
      ...
      <!-- ------------------ -->
        <div class="clr"></div>
          ...
          <div class="pagetitle icon-48-contact"><h2>Despesas da Rubrica</h2></div>
    </div></div>
    <!-- Filtro de busca -->
    ...
    <tbody>
      <tr>
        ...
        <td colspan="3"><?php echo $rubricaIDSaldo->nomeprojeto;?></td>
        <td><font size="2"><b>Orçamento:</b></font></td>
        <td colspan="3"> <?php echo number_format($rubricaIDSaldo->orcamento, 2, ',','.');?> </td>
      </tr>  
      <tr> 
        <td><font size="2"><b>Data Inicio:</b></font></td>
        <td colspan="3"> <?php databrRubricadeprojeto($rubricaIDSaldo->data_inicio);?> </td>
        <td><font size="2"><b>Data Fim:</b></font></td>
        <td colspan="3"> <?php databrRubricadeprojeto($rubricaIDSaldo->data_fim);?> </td>
      </tr> 
      <tr>  
         ...
         <td colspan="3">
            <?php //Acessa a tabela coordenadores para exibir o nome do coordenador ja definido
	             ...
	             foreach($coordenador_listas as $coordenador_final_listas){
                         ?><?php if ($coordenador_final_listas->id == $rubricaIDSaldo->coordenador_id) echo $coordenador_final_listas->nomeProfessor;?></option> <?php
                         }
            ?></td> 
        ...
        <td colspan="3">
            <?php //Acessa a tabela fomento para exibir o nome do fomento ja definido
	             ...
	             foreach($fomento_listas as $fomento_final_listas){
                         ?><?php if ($fomento_final_listas->id == $rubricaIDSaldo->agencia_id) echo $fomento_final_listas->sigla;?></option> <?php
                         }
            ?></td>
      </tr>
      ...
      <tr> 
        ...
        <td colspan="3"> <?php echo $rubricaIDSaldo->edital;?> </td>
        <td><font size="2"><b>Status:</b></font></td>
        <td colspan="3"> <?php echo $rubricaIDSaldo->status;?></td>
      </tr>
      </tbody>
    <!-- ////// /////////////////////////// //////-->    
...
<!--        <th width="6%" align="center"><font color="#FFC000">Valor Despesaprojeto</font></th>
	...
	$i = 0;
	foreach( $gerenciarDespesasrubricas as $gerenciarItemDespesas ){
		$i = $i + 1;
		if ($i % 2){
...
//                          else if ($gerenciarItemDespesas->tipo_pessoa == "2") echo "Jurídica";?></td>
	...
	<?php
     ...
     <br>Foi(foram) encontrado(s) <b><?php echo sizeof($gerenciarDespesasrubricas);?></b> despesa(s).
     <input name='task' type='hidden' value='gerenciarDespesasrubricas'>
     <input name='idRubrica' type='hidden' value='<?php echo $idRubrica;?>'><!--referencia o ID do PROJETO para edicao--> 
     <input name='idDespesaSelec' type='hidden' value='0'>
     <input name='idDespesarubrica' type='hidden' value=''>
...
// Criar novo registro
function addDespesarubrica($despesarubrica = NULL, $descricao, $idRubrica){
    $database	= JFactory::getDBO();
    $Itemid = JRequest::getInt('Itemid', 0);
	?>
...
function voltarForm(form){
   form.task.value = 'gerenciarDespesasrubricas';
   form.submit();
   return true;
}
</script>
...
<!-- --------------------------------------------------------------- -->
    ...
    <div id="toolbar-box"><div class="m"><div class="toolbar-list" id="toolbar">
                <div class="cpanel2">
                    <div class="icon" id="toolbar-save">
                        ...
                        <!--<a href="javascript:document.formCadastro.task.value='salvarDespesarubrica';document.formCadastro.submit()">-->
  ...
  <b>Como proceder: </b>
  <ul>
   <li>Preencha todos os campos com os dados da Despesaprojeto <font color="#FF0000">(* Campos Obrigatórios)</font>.</li>
  </ul>
   ...
   <input name='projeto_id' type='hidden' value='<?php echo $idRubrica;?>'><!--referencia o ID do PROJETO para novo-->
   <tr>
    ...
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>Valor Despesaprojeto:</b></font></td>
    ...
    <link rel="stylesheet" type="text/css" href="components/com_controleprojetos/template.css">
    <input name='idDespesarubrica' type='hidden' value='<?php if($despesarubrica) echo $despesarubrica->id;?>'>
    <input name='idRubrica' type='hidden' value='<?php echo $idRubrica;?>'>    
    <input name='task' type='hidden' value='salvarDespesarubrica'>
...
// Editar o registro selecionado pela variavel idDespesarubrica
function editarDespesarubrica($despesarubrica = NULL, $descricao){
    $database	= JFactory::getDBO();
    $Itemid = JRequest::getInt('Itemid', 0);
	?>
...
<!-- --------------------------------------------------------------- -->
  ...
  <ul>
   ...
   <tbody>
    <tr>
        <input name='id' type='hidden' value="<?php echo $despesarubrica->id;?>"><!--referencia o ID da DESPESA para edicao--> 
    </tr>
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>Rubrica:</b></font></td>
          <td colspan="2">
          <select name="rubrica_id" class="inputbox">
            ...
            <?php //Acessa a tabela rubricas para exibir em list box
	             ...
	             $rubrica_listas = $database->loadObjectList();
	             foreach($rubrica_listas as $rubrica_final_listas){
                         ?><option value="<?php echo $rubrica_final_listas->id;?>"<?php if ($rubrica_final_listas->id == $despesarubrica->rubrica_id) echo 'SELECTED';?>><?php echo $rubrica_final_listas->codigo;?> - <?php echo $rubrica_final_listas->nome;?></option><?php
                         }
            ?>
          </select>
         </td>
    </tr>
    <input name='projeto_id' type='hidden' value="<?php  echo $despesarubrica->projeto_id;?>"><!--referencia o ID do PROJETO para edicao--> 
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>Descrição:</b></font></td>
        <td><input maxlength="80" size="80" name="descricao" class="inputbox" value="<?php echo $despesarubrica->descricao;?>"></td>
    </tr> 
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>Valor Despesaprojeto:</b></font></td>
        <td><input maxlength="15" size="15" name="valor_despesa" class="inputbox" value="<?php echo $despesarubrica->valor_despesa;?>"></td>
    </tr> 
          <td><font size="2"><font color="#FF0000">*</font><b>Tipo Pessoa:</b></font></td>
    <td>
        <select name="tipo_pessoa" class="inputbox">
        <option value="1" <?php if ($despesarubrica->tipo_pessoa == "1") echo 'SELECTED';?>>Física</option>
        <option value="2" <?php if ($despesarubrica->tipo_pessoa == "2") echo 'SELECTED';?>>Jurídica</option>
        </select>
    </td>    
    <tr>
        <td><font size="2"></font> <b>Data Emissão:</b></font></td>
        <td><input maxlength="10" size="10" id="data_emissao" name="data_emissao" class="inputbox" value="<?php databrDespesa($despesarubrica->data_emissao);?>" /></td>
    </tr>
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>Identificação Nota Fiscal:</b></font></td>
        <td><input maxlength="15" size="15" name="ident_nf" class="inputbox" value="<?php echo $despesarubrica->ident_nf;?>"></td>
    </tr> 
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>Nota Fiscal:</b></font></td>
        <td><input maxlength="15" size="15" name="nf" class="inputbox" value="<?php echo $despesarubrica->nf;?>"></td>
    </tr> 
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>Identificação Cheque:</b></font></td>
        <td><input maxlength="15" size="15" name="ident_cheque" class="inputbox" value="<?php echo $despesarubrica->ident_cheque;?>"></td>
    </tr> 
    
    <tr>
        <td><font size="2"></font> <b>Data Emissão do Cheque:</b></font></td>
        <td><input maxlength="10" size="10" id="data_emissao_cheque" name="data_emissao_cheque" class="inputbox" value="<?php databr($despesarubrica->data_emissao_cheque);?>" /></td>
    </tr>
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>Valor Cheque:</b></font></td>
        <td><input maxlength="15" size="15" name="valor_cheque" class="inputbox" value="<?php echo $despesarubrica->valor_cheque;?>"></td>
    </tr> 
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>favorecido:</b></font></td>
        <td><input maxlength="60" size="60" name="favorecido" class="inputbox" value="<?php echo $despesarubrica->favorecido;?>"></td>
    </tr> 
    <tr>
        <td><font size="2"><font color="#FF0000">*</font> <b>CNPJ / CPF:</b></font></td>
        <td><input maxlength="15" size="15" name="cnpj_cpf" class="inputbox" value="<?php echo $despesarubrica->cnpj_cpf;?>"></td>
    </tr> 
    
   </tbody>
   </table>
    <link rel="stylesheet" type="text/css" href="components/com_controleprojetos/template.css">
    <input name='idDespesarubrica' type='hidden' value='<?php if($despesarubrica) echo $despesarubrica->id;?>'><!--referencia o ID da DESPESA para edicao--> 
    <input name='idRubrica' type='hidden' value='<?php if($despesarubrica) echo $despesarubrica->projeto_id;?>'><!--referencia o ID do PROJETO para edicao--> 
    <input name='task' type='hidden' value='salvarDespesarubrica'>
    <input name='buscaDescricao' type='hidden' value='<?php echo $descricao;?>'>
</form>
    <?php
...
//Cria Relatorio
function relatorioDespesarubrica($despesarubrica) {
    $database	= JFactory::getDBO();
    $Itemid = JRequest::getInt('Itemid', 0);
?>
<form method="post" name="form" enctype="multipart/form-data" action="index.php?option=com_controleprojetos&Itemid=<?php echo $Itemid;?>" method="post" >    
<div id="toolbar-box"><div class="m">
                        <div class="toolbar-list" id="toolbar">
                            ...
                            <div class="pagetitle icon-48-contact"><h2>Dados da Despesaprojeto</h2></div>
                      </div>
</div>
<table border="0" cellpadding="3" cellspacing="2" width="100%">
    <tbody>
       <tr>
            ...
            <td colspan="3"><?php echo $despesarubrica->id;?></td>
       </tr>  
       <tr>
            <td><font size="2"><b>Rubrica:</b></font></td>
            <td colspan="3"><?php echo $despesarubrica->rubrica_id;?></td>
       </tr>  
       <tr>
            <td><font size="2"><b>Projeto:</b></font></td>
            <td colspan="3"><?php echo $despesarubrica->projeto_id;?></td>
       </tr>  
       <tr>     
            <td><font size="2"><b>Descrição:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->descricao;?> </td>
       </tr>  
       <tr> 
            <td><font size="2"><b>Valor da Despesaprojeto:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->valor_despesa;?> </td>
       </tr>  
       <tr> 
            <td><font size="2"><b>Tipo Pessoa:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->tipo_pessoa;?> </td>
       </tr> 
       <tr> 
            <td><font size="2"><b>Data Emissão:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->data_emissao;?> </td>
       </tr> 
       <tr> 
            <td><font size="2"><b>Identificacão Cheque:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->ident_cheque;?> </td>
       </tr> 
       <tr> 
            <td><font size="2"><b>Data Emissão:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->data_emissao_cheque;?> </td>
       </tr> 
       <tr> 
            <td><font size="2"><b>Valor Cheque:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->valor_cheque;?> </td>
       </tr> 
       <tr> 
            <td><font size="2"><b>Favorecido:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->favorecido;?> </td>
       </tr> 
       <tr> 
            <td><font size="2"><b>CNPJ/CPF:</b></font></td>
            <td colspan="3"> <?php echo $despesarubrica->cnpj_cpf;?> </td>
       </tr> 
    </tbody>
</table>
<hr><!--linha fina-->
    <link rel="stylesheet" type="text/css" href="components/com_controleprojetos/template.css">
    <input name='idRubrica' type='hidden' value='<?php echo $despesarubrica->projeto_id;?>'>
    <input name='task' type='hidden' value='gerenciarDespesasrubricas'>
</form>
<?php
}
...
//Exclui o registro
function excluirDespesarubrica($idDespesarubrica){
  $database	= JFactory::getDBO();
  $sql = "DELETE FROM #__contproj_rubricadeprojeto WHERE id = $idDespesarubrica";
  $database->setQuery($sql);
  $database->Query();
...
function databrDespesarubrica($datasqlDespesarubrica) {
	if (!empty($datasqlDespesarubrica)){
	$p_dt = explode('-',$datasqlDespesarubrica);
    	$data_brDespesarubrica = $p_dt[2].'/'.$p_dt[1].'/'.$p_dt[0];
	return print $data_brDespesarubrica;
	}
}
//////////////////////////////////////////
...
//Salva o registro que foi criado ou editado (recebe salvarDespesaprojetoprojeto)
function salvarDespesarubrica($idDespesarubrica = ""){
  $database = JFactory::getDBO();
  ...
  if($idDespesarubrica == ""){//condicao se o cadastro for novo
      $sql = "SELECT * FROM #__contproj_rubricadeprojeto WHERE descricao = '$descricao'";
      $database->setQuery($sql);
      $repetido = $database->loadObjectList();
        if(!$repetido){
           $sql = "INSERT INTO #__contproj_rubricadeprojeto
     ...
     else if($idDespesarubrica == $id){//condicao se o cadastro for editado
         ...
         $sql = "SELECT * FROM #__contproj_rubricadeprojeto WHERE id = '$id', rubrica_id ='$rubrica_id', projeto_id ='$projeto_id',
     ...
     $database->setQuery($sql);
      $repetido = $database->loadObjectList();
        if(!$repetido){
             $sql = "UPDATE #__contproj_rubricadeprojeto SET id = '$id', rubrica_id ='$rubrica_id', projeto_id ='$projeto_id',
                    descricao ='$descricao', valor_despesa ='$valor_despesa', tipo_pessoa ='$tipo_pessoa',
                    data_emissao ='$data_emissao', ident_nf ='$ident_nf', nf ='$nf', ident_cheque ='$ident_cheque',
                    data_emissao_cheque ='$data_emissao_cheque', valor_cheque ='$valor_cheque', favorecido ='$favorecido',
                    cnpj_cpf ='$cnpj_cpf' WHERE  id = $idDespesarubrica";                 
             $database->setQuery($sql);
             if($database->Query()){
