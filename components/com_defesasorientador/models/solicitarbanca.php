...
{             
	...
	/**
	 * 
	 * Função que retorna a quantidade de créditos cumpridos pelo aluno
	 * 
	 * 
	 * @return Integer 
	 */
	public function getCreditos() {
		...
		$sql = "select sum(creditos) credito from j17_disc_matricula m, j17_disciplina d 
				where m.idDisciplina = d.id and conceito in ('A', 'B', 'C', 'AP') and m.idAluno = $idAluno
				group by idAluno";

		$database->setQuery($sql);
		
		$creditos = $database->loadObjectList();
		
		if (!sizeof($creditos)) return 0;
		
		return $creditos[0]->credito;
...
where a.orientador = p.id and a.id = $idAluno";
	...
	/**
	 * função que retorna, em ordem, as fases a serem cumpridas pelo aluno
	 * 
