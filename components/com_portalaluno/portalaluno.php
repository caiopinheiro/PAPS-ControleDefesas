<?php
	...
	include_once("components/com_portalaluno/autorizacao.php");		
	...
	switch ($task) {
		...
		case "addSolicitacao":
			$idAluno = JRequest::getVar('idAluno', false);
			$aluno = identificarAlunoID($idAluno);
			salvarSolicitacao($aluno);
		break;
		
		case "imprimirSolicitacao":
			$idSolic = JRequest::getVar('item', false);
			$solic = identificarSolic($idSolic);
			
			$idAluno = JRequest::getVar('al', false);
			$aluno = identificarAlunoID($idAluno);
			
			telaImpressaoSolic($solic, $aluno);
		break;
	...
	function identificarSolicitacao($idSolic) {
		$database =& JFactory::getDBO();	
		$sql = "SELECT * FROM #__solic_entrada WHERE id = '$idSolic'";
		$database->setQuery( $sql );
		$solicitacao = $database->loadObjectList();
	...
	function identificarSolic($idSolic){
		$database	=& JFactory::getDBO();
		$sql = "SELECT * FROM #__solic_entrada WHERE id = $idSolic";
		$database->setQuery( $sql );
		$solic = $database->loadObjectList();
...
function aproveitarDisciplina($aluno, $idPeriodo) {
    ...
    <?php
	   ...
	   $sql = "SELECT A.idAproveitamento, A.tipoAproveitamento, A.idDisciplina, D.codigo, D.nomeDisciplina, D.creditos, D.carga, DATE_FORMAT(A.dataPedido, '%d/%m/%Y') AS dataPedido, A.comprovante, A.periodoCursado, A.conceito, A.frequencia, A.tipoAproveitamento, A.disciplina, A.codigo AS codDisc, A.ementa, A.carga_horaria FROM #__disciplina AS D JOIN #__aproveitamentos AS A ON D.id = A.idDisciplina WHERE idAluno = $aluno->id AND resultado = 0 ORDER BY codigo";
	   $database->setQuery($sql);
	   $myDisciplinas = $database->loadObjectList(); 
		...
		<table width="100%" border="0" cellspacing="2" cellpadding="2">
		   ...
		   foreach ($myDisciplinas as $d1) {
		   
		   
					if (($d1->tipoAproveitamento == '1') || ($d1->tipoAproveitamento == '3')) { 
           ?>         
                    <tr>
                      <td align="center"><a href="javascript:ConfirmarExclusao(document.formAproveitamento, <?php echo $d1->idAproveitamento;?>)"> <img src="components/com_portalaluno/images/lixeira.gif"  alt="excluir" title="excluir"/> </a></td>
					...
					?>
                        ...
                        <tr>
                          ...
                          <td align="center"><?php echo $d1->dataPedido; ?></td>
                          <td align="center"><?php echo $d1->codDisc; ?></td>
                          <td align="center"><?php echo $d1->disciplina; ?> <a href="<?php echo $d1->ementa;?>" target="_blank"><img src="components/com_portalaluno/images/icon_pdf.gif" width="16" height="16" border="0"></a></td>
                          <td align="center"><?php echo $d1->periodoCursado; ?></td>
                          <td align="center"><?php echo $d1->creditos; ?></td>
                          <td align="center"><?php echo $d1->carga_horaria; ?></td>
                          <td align="center"><?php echo $d1->frequencia; ?></td>
                          <td align="center"><?php echo $d1->conceito; ?></td>
                        </tr>
	...
	<b>Como proceder: </b>
    <ul>
	<li>Selecione primeiramente a disciplina do PPGI a ser aproveitada e em seguida escolha o tipo de aproveitamento.</li>
	<li>Caso a disciplina tenha sido cursada como TRANSFER&#202;NCIA DE REALIZA&#199;&#195;O em outra IES (tipos 2, 4, 5 e 6), voc&#234; dever&#225; preencher os dados da disciplina cursada na outra IES (c&#243;digo, nome, carga hor&#225;ria, cr&#233;ditos e ementa).</li>
	</ul>
	...
	<table width="100%" cellpadding="2" cellspacing="2" id="tbOutra" style="display:none; background-color:#E4E4E4; border:1px solid #999; border-radius:5px;">
        <tr>
            <td class="cellCommom" colspan="2" align="center"><b>Dados da Disciplina na outra IES - TRANSFER&#202;NCIA DE REALIZA&#199;&#195;O<b></td>
        </tr>
		<tr>
	...
	<table width="100%">
        ...
        <tr>
            <td bgcolor="#CCCCCC"><strong>Frequ&#234;ncia xxx.xx%:</strong></td>
            <td><input type="text" id="frequencia" name="frequencia" size="6" maxlength="6" onkeyup="formatAsDollars(this);"/></td>
        </tr>
        <tr>
