...
function historicoAluno($aluno) {
	...
	$fasesPesquisa = array (1 => '1. Definição do Tema', 2 => '2. Preparação e Apresentação do Projeto', 3 => '3. Pesquisa Bibliográfica', 4 => '4. Desenvolvimento de Projeto', 5 => '5. Conclusão', 6 => '6. Redação Final', 7 => '7. Apresentação');
    ...
    /*Busca um trancamento deferido*/
    $sql = "SELECT * FROM #__trancamentos WHERE idAluno = $aluno->id AND status='4'";
    $database->setQuery( $sql );
    ...
    $ingresso = strftime("%m/%Y", strtotime($aluno->anoingresso));
    $pdf->addText(280,450,10,"<b>Mês/Ano de Ingresso:</b> $ingresso",0,0);
    $pdf->ezText('');  //Para quebra de linha
...
//    $pdf->addText(495,725,8,"<b>Hora:</b> ".date("H:i"),0,0);
    ...
    $pdf->ezText('');  //Para quebra de linha
    $pdf->ezText('');  //Para quebra de linha
    $pdf->ezText('<b>Legenda:</b> A = Excelente	| B = Ótimo	| C = Bom | R = Reprovado | X = Trancamento',8,$optionsText);
    
    //caso haja um trancamento deferido
    if($trancamentos <> NULL){
    
    	foreach( $trancamentos as $t )
    	{
    		//calcula o periodo a partir da data de aprovaÃ§Ã£o do trancamento
    		$data = explode("-", $t->dataInicio);
    		$mesPeriodo = (int) $data[1];
    		if (($mesPeriodo >= 01) && ($mesPeriodo <= 06)) {
    			$periodo = $data[0]."/01";
    		} else{
    			$periodo = $data[0]."/02";
    		}
    		...
    		$trancamento[] = array('Periodo'=>$periodo,'Status'=> "Deferido",'Justificativa'=>utf8_decode($t->justificativa));
    	}
    
    	$pdf->ezText('');  //Para quebra de linha
    	$pdf->ezText('<b>PEDIDOS DE TRANCAMENTO</b>',11,$optionsText);
    	$pdf->ezText('');  //Para quebra de linha
    	$pdf->ezTable($trancamento,$cols,'',$optionsTable);
    }
    ...
    $pdf->addText(110,100,8,'<b>Profª. Drª. Eulanda Miranda dos Santos</b>',0,0);
    $pdf->addText(125,90,8,'Coordenadora do Programa',0,0);
    $pdf->addText(360,100,8,'<b>Prof. Dr. Afonso Duarte Leão de Souza</b>',0,0);
    $pdf->addText(330,90,8,'Diretor do Departamento de Pós-Graduação - PROPESP',0,0);
