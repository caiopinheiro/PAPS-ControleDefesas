...
include_once("components/com_portalsecretaria/selecaoacre.php");
...
include_once("components/com_portalsecretaria/autorizacao.php");
include_once("components/com_portalsecretaria/funcionarios.php");
...
switch ($task) {
  ...
  case "candidatosacre":
	$periodo = JRequest::getVar('periodo');
	avaliarCandidatosAcre($periodo);
  break;
  
  case "aprovar":
	$periodo = JRequest::getVar('periodo');
	$idCandidato = JRequest::getVar('idcandidato');
	aprovarCandidato($idCandidato, $periodo);
  break;
 ...
 case "pendenciasalunos":
	listarPendenciasAlunosPPGI();
  break;
  ...
  case "abrirInscricao":	
  	  ...
  	  $sql = "UPDATE #__periodos SET inscricao = '1' WHERE id = '$idPeriodo'";
	  $database->setQuery($sql);
	  $res = $database->Query();
	  
      // ID REFERENTE AO SUBITEM FORMULARIO DE INSCRIÇÃO
	  $sql2 = "UPDATE #__menu SET published = 1 WHERE id = '176'";
  ...
  case "encerrarInscricao":	  
   	  $idPeriodo = JRequest::getVar('idHidden');
	  
  	  $sql = "UPDATE #__periodos SET inscricao = '0' WHERE id = '$idPeriodo'";
	  $database->setQuery($sql);
	  $res = $database->Query();
	  
	  // ID REFERENTE AO ITEM FORMULARIO
	  $sql2 = "UPDATE #__menu SET published = 0 WHERE id = '176'";
	  $database->setQuery($sql2);
	  $res2 = $database->Query();
  ...
  case "imprimirFolha":
	  ...
	  $tipoDefesa = JRequest::getVar('tipoDefesa');
	  if($tipoDefesa == "Q")
            imprimirFolhaQualificacao($idAluno, $tipoDefesa);
         else
  ...
  // ------------------- GERENCIAMENTO DE MEMBROS EXTERNOS DE BANCA -------------------
  case "membrosbanca":
	$nome = JRequest::getVar('buscaNome');
	listarMembrosExternos($nome);
  break;
  
  case "addMembroExterno":
	  $nome = JRequest::getVar('buscaNome');
	  editarMembroExterno(NULL, $nome);
  break;
  
  case "editarMembroExterno":
	  $nome = JRequest::getVar('buscaNome');
	  $idMembro = JRequest::getVar('idMembro');
	  $membro = identificarMembroID($idMembro);
	  editarMembroExterno($membro, $nome);
  break;
  
  case "salvarMembroExterno":
	  $nome = JRequest::getVar('buscaNome');
	  $idMembro = JRequest::getVar('idMembro');
	  
	  if(salvarMembroExterno($idMembro))
	  	listarMembrosExternos($nome);
	  else {
	  	$nome = JRequest::getVar('buscaNome');
	  	$membro = identificarMembroID($idMembro);
  ...
  case "excluirMembroExterno":
		$nome = JRequest::getVar('buscaNome');
		$idMembro = JRequest::getVar('idMembro');
		excluirMembroExterno($idMembro);
		listarMembrosExternos($nome);
  break;
  
  case "verMembroExterno":
	  $idMembro = JRequest::getVar('idMembro');
	  $membro = identificarMembroID($idMembro);
	  relatorioMembroExterno($membro);
  break;
  
  
  // ------------------- TASKS DE APROVEITAMENTO -------------------
  ...
  // ------------------- TASKS REFERENTES A JUBILAMENTO -------------------
  case "jubilamento":
  ...
  // ------------------------ TRANCAMENTO ------------------------
	...
	case "cadFuncionario":
		$idFunc = JRequest::getCmd('idFunc', false);
		salvarFuncionario($idFunc);
	break;
	
	case "editFuncionario":	
		$idFunc = JRequest::getVar('idFunc');
		atualizarFuncionario($idFunc);
	break;
	
	case "editarFuncionario":	
		$idFunc = JRequest::getVar('idFunc');
		$funcionario = identificarFuncionarioID($idFunc);
		telaEditarFuncionario($funcionario);
	break;
	...
	case "verFuncionario":
		$idFunc = JRequest::getVar('idFunc');
		$funcionario = identificarFuncionarioID($idFunc);
		relatorioFuncionario($funcionario);
	break;

 	// Carta de Recomendacao
	case "reenviarCartas":
	  	$idCandidato = JRequest::getVar('idCandidato');
	  	reenviarCartasRecomendacao($idCandidato);
		$periodo = JRequest::getVar('periodo');
		avaliarCandidatos($periodo);
	break;
...
function mensagemDeErro($text){ ?>
  ...
  <table style='width: 100%;' border='1' cellspacing='2'>
      <tbody>
          ...
          <tr>
            <td width='48px'>
              <img style='width: 48px; height: 48px;' alt='erro' src='components/com_portalsecretaria/images/erro.gif'>
            </td>
