...
//-->
    ...
    <div class="clr"></div>
          ...
          <div class="pagetitle icon-48-contact"><h2>Candidatos do PPGI</h2></div>
    </div></div>
	...
	<thead>
      <tr bgcolor="#002666">
        ...
        <th width="3%" align="center"><font color="#FFC000">Cartas Recebidas</font></th>
        <th width="3%" align="center"><font color="#FFC000">Cartas Restantes</font></th>
	...
	?>
		...
		<td width='16'><div align="center">
        <?php
			$cartasRecebidas = cartaRecomendacao($candidato->id, "recebida");
			foreach($cartasRecebidas as $carta){
			
             //$idCarta = cartaRecomendacao(1, $candidato->id);
             //if($idCarta){
        ?>
            <a href="index.php?option=com_inscricaoppgi&Itemid=176&task=imprimirCartas&idCandidato=<?php echo $candidato->id;?>&idCarta=<?php echo $carta->id;?>" onclick="window.open(this.href,'targetWindow','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,');return false;"><img border='0' src='components/com_portalsecretaria/images/b_view.png' width='16' height='16' title='Visualizar Carta de Recomenda&#231;&#227;o'></a>			</a>
        <?php } ?>
		</div></td>
		<td width='16'><div align="center">
        <?php
			$cartasAtrasadas = cartaRecomendacao($candidato->id, "atrasada");
			if($cartasAtrasadas){
		?>
			<a href="index.php?option=com_portalsecretaria&Itemid=176&task=reenviarCartas&idCandidato=<?php echo $candidato->id;?>"><?php echo sizeof($cartasAtrasadas);?> <img border='0' src='components/com_portalsecretaria/images/emailButton.png' width='16' height='16' title='Reenviar Pedidos de Carta de Recomenda&#231;&#227;o'></a>
        <?php } ?>
		</div></td>
		...
		<td align="left"><img border='0' src='<?php echo $resultado[$candidato->resultado];?>' width='16' height='16' title='<?php echo $resultadoTitle[$candidato->resultado];?>'> <?php echo $candidato->nome;?></td>
...
<tr style="background-color: rgb(113, 150, 216);">
 ...
 ////////////////////////////////////////////////////////////////////
function cartaRecomendacao($idCandidato, $tipo){
...
function reenviarCartasRecomendacao($idCandidato){
	...
	$prazo = date("Y-m-d", strtotime('+4 days'));

	    foreach($recomendacoes as $carta){
		$sql = "UPDATE #__recomendacoes SET prazo = '$prazo', dataEnvio = '0000-00-00 00:00:00' WHERE token = '$carta->token'";
		$database->setQuery( $sql );
		$database->Query();

		notificarCartaRecomendacao($carta->token);
	}
	JFactory::getApplication()->enqueueMessage(JText::_('Envio realizado com sucesso.'));


}

function notificarCartaRecomendacao($token){

    global $database;
	$database	=& JFactory::getDBO();

    $sql = "SELECT R.nome,R.email,R.token,R.prazo,R.idCandidato, C.nome as nomeCandidato, C.email as emailCandidato FROM #__recomendacoes AS R JOIN #__candidatos AS C ON R.idCandidato = C.id WHERE token = '$token'";
    $database->setQuery( $sql );
	$recomendacoes = $database->loadObjectList();
    ...
    $message .= "\nPara isso, a carta deve ser preenchida eletronicamente utilizando o link: \n http://www.icomp.ufam.edu.br/index.php?option=com_inscricaoppgi&task=rec&token=".$recomendacoes[0]->token."\r\n";
    $message .= "O prazo que você tem para preencher a carta é ".$recomendacoes[0]->prazo.".\r\n";
    $message .= "Em caso de dúvidas, por favor nos contate.\r\n";
    $message .= "\nCoordenação do PPGI - ".date(DATE_RFC822)."\r\n";
    $message .= $mime_boundary."\r\n";

    $mail =& JFactory::getMailer();
    $mail->addRecipient( $recomendacoes[0]->email );
    $mail->setSender( array( "secppgi@ufam.edu.br", "Coordenacao do PPGI" ) );
