...
class DefesascoordenadorController extends JController {
	...
	public function enviarConvite($pathConvite){
		...
		if($emails != null){			
			...
			// message
			$message .= "A CoordenaÃ§Ã£o do Programa de PÃ³s-graduaÃ§Ã£o em InformÃ¡tica PPGI/UFAM tem o prazer de convidÃ¡-lo para a sessÃ£o pÃºblica de apresentaÃ§Ã£o da Defesa de DissertaÃ§Ã£o/Tese de Mestrado/Doutorado:\r\n\n";
			$message .= "$titulo\r\n\n";
			...
			$message .= "DATA: ".$data."\r\n";
			$message .= "HORÃ�RIO: ".$defesa[0]->horario."\r\n\n";
			...
			return JUtility::sendMail($user->email, "IComp: Controle de Defesas", $emails, $subject, $message, false, NULL, NULL, $path);
		}
	}
	...
	*/
	...
	public function gerarConviteDefesa() {
		...
		$pdf->MultiCell(0,5,utf8_decode('CONVITE Ã€ COMUNIDADE'),0, 'C');
		$pdf->MultiCell(0,5,"",0, 'C');
		
		$tag = " A CoordenaÃ§Ã£o do Programa de PÃ³s-GraduaÃ§Ã£o em InformÃ¡tica PPGI/UFAM tem o prazer de convidar toda a comunidade para a sessÃ£o pÃºblica de apresentaÃ§Ã£o de defesa de";

		if ($defesa[0]->tipoDefesa == 'Q1' OR $defesa[0]->tipoDefesa == 'Q2' ) {
			if ($aluno[0]->curso == 2){
				$tag = $tag . " exame de qualificaÃ§Ã£o de doutorado:";
				$chave .= "_defesa_qualific_doutorado_convite";
			}
			else{
				$tag = $tag . " exame de qualificaÃ§Ã£o de mestrado:";
		...
		$pdf->MultiCell(0,5,"",0, 'C');
		$pdf->MultiCell(0,6,utf8_decode("HORÃ�RIO: ") . utf8_decode($defesa[0]->horario),0, 'J');
		$pdf->MultiCell(0,5,"",0, 'C');
		...
		$pdf->MultiCell(0,4,utf8_decode("Coordenadora do Programa de PÃ³s-GraduaÃ§Ã£o em InformÃ¡tica PPGI/UFAM"),0, 'C');
