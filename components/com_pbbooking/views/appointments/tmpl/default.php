...
<table class="pbbooking-appointment-table">
	...
	<?php else:?>
		<?php foreach ($this->events as $event) :?>
			<tr>
				<td><?php echo JHTML::_('date',date_create($event->dtstart, new DateTimeZone(PBBOOKING_TIMEZONE))->format(DATE_ATOM),JText::_('COM_PBBOOKING_SUCCESS_DATE_FORMAT').' @ '.JText::_('COM_PBBOOKING_SUCCESS_TIME_FORMAT'));?></td>
				<td>
					<?php 
						$dt_diff = (int)date_create($event->dtstart,new DateTimeZone(PBBOOKING_TIMEZONE))->format('U') - (int)date_create("now",new DateTimeZone(PBBOOKING_TIMEZONE))->format('U');
						echo sprintf('%0.0f',(($dt_diff/60)/60)).' '.JText::_('COM_PBBOOKING_APPOINTMENTS_SELFSERVICE_HOURS');
					?>
				</td>
				<td><?php echo $event->name;?></td>
				<td><?php echo $event->cal_name;?></td>
				<td>
